...
const DashboardCards = () => {
    return (
        <div className=" bg-black text-white flex flex-col justify-start items-center p-4">
            {/* <div className="text-center mb-8">
                <div className="text-4xl mb-2">0</div>
                <div className="text-lg">$GLOB ACQUIRED</div>
            </div> */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 -ml-56">
                {cards.map((card) => (
                    <div key={card.id} className="p-4 w-44 h-40 bg-black/40 border border-white rounded-lg shadow-lg flex flex-col items-center">
                        <div className="text-lg mb-2">{card.points}</div>
                        <div className={`px-2 py-1 rounded-full ${card.status === 'Claim' ? 'bg-orange-500' : 'bg-gray-600'} mb-2`}>
                            {card.status}
                        </div>
